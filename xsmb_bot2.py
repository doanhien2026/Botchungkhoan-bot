# ==========================================================
# BOT XSMB — V6.3 | NGUỒN DỮ LIỆU CHÍNH XÁC | DỰ ĐOÁN NGAY
...
@app.route('/')
def home():
    return "✅ Bot XSMB V6.3 — Nguồn dữ liệu chính xác | Dự đoán ngay"
...
def fetch_result(date_str):
    """Lấy kết quả XSMB từ API chính xác"""
    d, m, y = date_str.split("/")
    date_api = f"{d}/{m}/{y}"
    
    # Nguồn 1: API xosomienbac.com.vn — đáng tin cậy
    try:
        url = f"https://api.xosomienbac.com.vn/api/result?date={date_api}"
        r = requests.get(url, headers=HEADERS, timeout=10)
        if r.status_code == 200:
            data = r.json()
            if data.get("status") == "success" and data.get("data"):
                res_data = data["data"]
                dac_biet = str(res_data.get("special", "")).strip()
                g1 = str(res_data.get("prize1", "")).strip()
                loto_list = res_data.get("loto", [])
                if dac_biet and len(dac_biet) == 5 and g1 and len(g1) == 5:
                    lotos = [str(n)[-2:] for n in loto_list] if loto_list else []
                    if not lotos:
                        # Tạo danh sách lô từ tất cả các giải
                        all_prizes = res_data.get("prizes", [])
                        for p in all_prizes:
                            val = str(p).strip()
                            if len(val) >= 2:
                                lotos.append(val[-2:])
                    lotos = sorted(list(set(lotos)))[:27]
                    return {
                        "source": "API-XOSOMIENBAC",
                        "special": dac_biet,
                        "g1": g1,
                        "loto": lotos
                    }
    ...
    try:
        ...
        print(f"❌ Cả 2 nguồn lỗi: {e}")
        return None

# ====================== 🧠 DỰ ĐOÁN — TRẢ VỀ NGAY LẬP TỨC ======================
...
def gen_prediction(days=60, target_date=None):
    """Tạo dự đoán — LUÔN TRẢ VỀ NGAY, KHÔNG ĐỢI"""
    ...
    lines = [
        ...
        f"📅 Dựa trên {days} ngày gần nhất | Nguồn: API + XSDAIPHAT",
...
def cmd_start(m):
    ...
    bot.send_message(m.chat.id,
        "🤖 **BOT XSMB — THỐNG KÊ SỐ LÔ V6.3**\n"
        "✅ Nguồn dữ liệu: API chính xác + XSDAIPHAT\n"
        "✅ Dự đoán trả về ngay lập tức!\n\n"
...
def handle(m):
    ...
    try:
        ...
        return bot.send_message(m.chat.id, "❌ Ngày không hợp lệ!")
    date_str = f"{d}/{mo}/{y}"
    bot.send_message(m.chat.id, f"🔍 Đang lấy & lưu **{date_str}** từ nguồn chính xác...")
    res = fetch_result(date_str)
    if not res:
        return bot.send_message(m.chat.id, f"⚠️ **CHƯA CÓ KẾT QUẢ — {date_str}**\n(Chưa đến giờ quay hoặc lỗi nguồn)")
...
if __name__ == "__main__":
    print("="*60)
    print("🚀 BOT XSMB — V6.3 | NGUỒN DỮ LIỆU CHÍNH XÁC")
    ...
    print("✅ BOT SẴN SÀNG — /dudoan trả về kết quả NGAY!")
    print("="*60)
