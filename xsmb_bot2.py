...
# ============================================================
# CẤU HÌNH — ✅ ĐÃ ĐIỀN SẴN TOKEN & ID CỦA BẠN
# ============================================================
...
# ⚙️ TÙY CHỌN: True = tính lại số mới / False = khóa không đổi
...
def send_telegram(message):
    ...
    try:
        ...
        if response.status_code != 200:
            logging.error(f"Telegram lỗi {response.status_code}")
            return False
        return True
...
def init_database():
    ...
    if FORCE_RECALCULATE:
        logging.warning("⚠️ Đang xóa dữ liệu cũ để tính lại mới...")
...
def save_results(results):
    ...
    con.commit()
    con.close()
    logging.info(f"✅ Đã lưu {len(results)} ngày dữ liệu")
...
def calculate_scores(history):
    ...
    for num, vals in matrix.items():
        ...
        score = (0.20 * f60 + 0.20 * f30 + 0.10 * f14 + 0.10 * f7 +
                 0.25 * fall60 + 0.10 * fall30 + 0.05 * gan_score)
        scores[num] = {
            "score": score,
            "f60": f60,
            "f30": f30,
...
def backtest(history):
    if len(history) < 65:
        return {"loto_rate": 0, "xien_rate": 0, "dau_rate": 0}
    ...
    for i in range(start, len(history)):
        ...
        if any(n in actual for n in pred["loto"]):
            loto_hits += 1
        if pred["xien"][0] in actual and pred["xien"][1] in actual:
            xien_hits += 1
        db = history[i]["db"]
        if db and db[-2:] == str(pred["dau"]):
            dau_hits += 1
        total += 1
    return {
...
def get_locked_signal(target_date):
    ...
    cur.execute("""
        ...
        FROM signals WHERE target_date=?
    """, (target_date,))
    row = cur.fetchone()
    con.close()
    if not row:
...
def lock_signal(target_date, prediction, bt):
    ...
    """, (
        ...
        datetime.now(VIETNAM_TZ).isoformat(),
        bt["loto_rate"], bt["xien_rate"], bt["dau_rate"]
    ))
    con.commit()
    con.close()
    return {
...
# ============================================================
# GỬI TÍN HIỆU — ĐỊNH DẠNG CHÍNH XÁC
# ============================================================
...
🔥 3 lô: {"✅ CÓ TRÚNG" if loto_hit else "❌ KHÔNG TRÚNG"}
🎯 Xiên 2: {"✅ CÓ TRÚNG" if xien_hit else "❌ KHÔNG TRÚNG"}
🎲 Đầu đề: {"✅ CÓ TRÚNG" if dau_hit else "❌ KHÔNG TRÚNG"}
"""
    send_telegram(msg)
...
def job_1900():
    ...
    try:
        ...
        if len(history) < LOOKBACK:
            send_telegram(f"⚠️ Cần ít nhất {LOOKBACK} ngày dữ liệu, mới có {len(history)} ngày")
            return
...
def main():
    ...
    scheduler.add_job(job_1835, "cron", hour=18, minute=35, id="job_r1")
    scheduler.add_job(job_1900, "cron", hour=19, minute=0, id="job_r2")
    scheduler.add_job(job_every_5_minutes, "cron", minute="*/5", id="job_r3")
