# ==========================================================
# BOT XSMB — V7.1 | 3 NGUỒN DỮ LIỆU + NHẬP THỦ CÔNG
...
@app.route('/')
def home():
    return "✅ Bot XSMB V7.1 — 3 Nguồn + Nhập thủ công"
...
# NGUỒN 1: XOSO.ME — Nhanh, ít bị chặn
def fetch_from_xosome(date_str):
    ...
    try:
        ...
        if validate_result(special, g1, loto):
            print(f"✅ LẤY TỪ XOSO.ME — ĐB: {special}, G1: {g1}, Lô: {len(loto)} số")
            return {"source": "XOSO.ME", "special": special, "g1": g1, "loto": loto}
        return None
    except Exception as e:
        print(f"❌ Lỗi XOSO.ME: {e}")
        return None

# NGUỒN 2: KQSXOSO.COM — Dự phòng 1
def fetch_from_kqxs(date_str):
    ...
    try:
        ...
        loto = sorted(list(loto_set))
        if validate_result(special, g1, loto):
            print(f"✅ LẤY TỪ KQSXOSO — ĐB: {special}, G1: {g1}, Lô: {len(loto)}")
            return {"source": "KQSXOSO", "special": special, "g1": g1, "loto": loto}
        return None
    except Exception as e:
        print(f"❌ Lỗi KQSXOSO: {e}")
        return None

# NGUỒN 3: XOSODAIPHAT — Dự phòng 2
def fetch_from_xosodaiphat(date_str):
    ...
    try:
        ...
        loto = sorted(list(loto_set))
        if validate_result(special, g1, loto):
            print(f"⚠️ DÙNG XOSODAIPHAT — ĐB: {special}, G1: {g1}")
...
def fetch_result(date_str):
    ...
    if result:
        return result
    print("⚠️ Nguồn 1 thất bại → thử KQSXOSO...")
    result = fetch_from_kqxs(date_str)
    if result:
        return result
    print("⚠️ Nguồn 2 thất bại → thử XOSODAIPHAT...")
    result = fetch_from_xosodaiphat(date_str)
    if result:
        return result
    print("❌ TẤT CẢ 3 NGUỒN ĐỀU THẤT BẠI")
    return None
...
💡 Hướng dẫn nhanh:
   • Gõ: 29082026 → tự lấy kết quả
   • Hoặc: /nhap 29082026 90737 47583 01,03,08,...
...
def cmd_start(m):
    bot.send_message(m.chat.id,
        "🤖 **BOT XSMB — THỐNG KÊ SỐ LÔ V7.1**\n"
        "✅ 3 Nguồn dữ liệu: XOSO.ME | KQSXOSO | XOSODAIPHAT\n"
        "✅ Lệnh /nhap — Nhập thủ công khi nguồn bị chặn\n"
        "✅ Hiển thị ngày dự đoán + Tỷ lệ rõ ràng\n"
        f"✅ Phân tích {ANALYSIS_DAYS} ngày lịch sử\n\n"
        "📌 Gõ DDMMYYYY → Tự lấy & lưu kết quả (VD: 29082026)\n"
        "📌 /nhap DDMMYYYY ĐB G1 LÔ1,LÔ2,... → Nhập thủ công\n"
...
# LỆNH NHẬP THỦ CÔNG — Khi tất cả nguồn bị chặn
...
def cmd_nhap_thu_cong(m):
    ...
    if saved:
        ...
        rep += f"🏆 Đặc Biệt: `{db}`\n"
        rep += f"🥇 Giải Nhất: `{g1}`\n"
        rep += f"🎯 Lô về: {', '.join(f'`{n}`' for n in loto)}\n"
        rep += "\n⚠️ Chỉ tham khảo — Chơi có trách nhiệm!"
...
def handle(m):
    ...
    if not re.match(r"^\d{8}$", txt):
        return bot.send_message(m.chat.id, "⚠️ Gõ DDMMYYYY để lưu kết quả (VD: 29082026)\nHoặc /nhap để nhập thủ công khi nguồn bị chặn!\nGõ /start để xem hướng dẫn đầy đủ.")
    ...
    if not res:
        return bot.send_message(m.chat.id,
            ...
            "Tất cả 3 nguồn đều không trả dữ liệu.\n"
            f"👉 Dùng lệnh: /nhap {txt} ĐB G1 LÔ1,LÔ2,... để nhập thủ công!",
            parse_mode="Markdown"
        )
...
if __name__ == "__main__":
    print("="*60)
    print("🚀 BOT XSMB — V7.1 | 3 NGUỒN + NHẬP THỦ CÔNG")
    print("✅ XOSO.ME | ✅ KQSXOSO | ✅ XOSODAIPHAT | ✅ /nhap thủ công")
    print("="*60)
    bot.remove_webhook()
