# ==========================================================
# BOT XSMB — DỰ ĐOÁN THEO LOGIC TẦN SUẤT + CHU KỲ NGỦ
# ✅ Định dạng giống ảnh | ✅ Tính toán chi tiết | ✅ Dữ liệu thực
# ==========================================================
...
def calc_top3_loto(lotos, history):
    """Tính 3 con lô: tần suất + chu kỳ ngủ → ĐÚNG LOGIC ẢNH"""
    if not lotos: return []
    freq = Counter(lotos)
    total = len(lotos)
    # Tính chu kỳ ngủ = số ngày chưa xuất hiện
    ...
    for idx, dt in enumerate(sorted_dt):
        ...
        for n in nums:
            if n not in last_appear:
                last_appear[n] = idx  # idx = số ngày ngủ
    # Tính điểm & sắp xếp
    scored = []
    ...
    # Sắp xếp theo tần suất giảm dần → lấy top 3
    scored.sort(key=lambda x: (-x["count"], -x["rate"]))
    return scored[:3]

def gen_prediction(days=60):
    """Tạo báo cáo ĐÚNG ĐỊNH DẠNG NHƯ ẢNH"""
    ...
    # 1 cặp lô xiên = 2 con cao nhất
    if len(top3) >= 2:
        xien_num1, xien_num2 = top3[0]["num"], top3[1]["num"]
    elif len(top3) == 1:
        xien_num1, xien_num2 = top3[0]["num"], "99"
    else:
        xien_num1, xien_num2 = "00", "00"
    ...
    lines.extend([
        "",
        "🔀 **1 CẶP LÔ XIÊN:**",
        f"   → Kết hợp 2 con cao nhất: `{xien_num1} – {xien_num2}`",
        "",
        "🔢 **DỰ KIẾN ĐẦU SỐ ĐỀ:**",
...
def cmd_start(m):
    ...
    bot.send_message(m.chat.id,
        "🤖 **BOT XSMB — DỰ ĐOÁN T989**\n"
        "✅ Logic: Tần suất + chu kỳ ngủ | 📡 2 nguồn dữ liệu\n"
    ...
    while True:
        try:
            ...
            if now.hour == 18 and 35 <= now.minute <= 45 and last != today:
                ...
                # Gửi dự đoán ngày mai — ĐÚNG ĐỊNH DẠNG ẢNH
                if pred:
                    d, m, y = today.split("/")
...
# ====================== 🚀 KHỞI ĐỘNG ======================
@app.route('/')
def home(): return "✅ BOT XSMB T989 — ĐANG HOẠT ĐỘNG", 200
def run_flask(): app.run(host='0.0.0.0', port=PORT)

if __name__ == "__main__":
    print("="*60)
    print("🚀 BOT XSMB — DỰ ĐOÁN THEO LOGIC ẢNH")
    print("📡 2 Nguồn | 💾 Tự lưu | ⏰ Auto 18:35 | 🧠 Đúng định dạng")
    print("="*60)
    bot.remove_webhook()
    time.sleep(1)
    Thread(target=run_flask, daemon=True).start()
    Thread(target=auto_send, daemon=True).start()
    print("✅ SẴN SÀNG — LOGIC & ĐỊNH DẠNG ĐÃ KHỚP VỚI ẢNH")
    print("="*60)
    while True:
        try:
